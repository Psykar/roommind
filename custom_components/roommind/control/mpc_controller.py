...
class MPCController:
    ...
    async def async_apply(
        ...
        *,
        ...
        await self._sync_device_presets(
            thermostats + self.acs,
            desired_preset=device_preset_mode,
        )
    ...
    async def _sync_device_preset(self, entity_id: str, *, desired_preset: str | None) -> None:
        ...
        if current_preset != PRESET_ECO:
            return
        if "none" not in preset_modes:
            return
        await self._call("set_preset_mode", {"entity_id": entity_id, "preset_mode": "none"})
