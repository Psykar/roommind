...
class TestPresenceDetection:
    ...
    @pytest.mark.asyncio
    async def test_presence_away_off_passes_no_device_preset_hint(self, hass, mock_config_entry):
        """presence_away_action='off' must not force an eco preset on child devices."""
        store = _make_store_mock({"living_room_abc12345": SAMPLE_ROOM})
        store.get_settings.return_value = {
            "presence_enabled": True,
            "presence_persons": ["person.kevin"],
