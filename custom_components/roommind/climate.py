...
class RoomMindRoomClimate(_RoomMindBaseClimate):
    ...
    def target_temperature(self) -> float:
        ...
        if live is not None:
            return live
        room = self._get_room() or {}
        override_temp = self._floatish(room.get("override_temp"))
...
class RoomMindOverrideClimate(RoomMindRoomClimate):
    ...
    def target_temperature(self) -> float:
        ...
        room_override_live = build_override_live(self._get_room() or {})
        room_override_temp = self._floatish(room_override_live.get("override_temp"))
        if room_override_temp is not None:
            return room_override_temp

        if self._is_override_active():
            live_target = self._floatish(self._get_live_climate().get("target_temperature"))
            if live_target is not None:
                return live_target
            override_heat = self._floatish(room_override_live.get("override_heat_temp"))
            if override_heat is not None:
                return override_heat
            override_cool = self._floatish(room_override_live.get("override_cool_temp"))
